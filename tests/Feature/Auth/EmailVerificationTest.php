...
test('email can be verified', function (): void {
    ...
    $response->assertRedirect(route('trading', absolute: false).'?verified=1');
});
...
test('verified user is redirected to trading from verification prompt', function (): void {
...
test('already verified user visiting verification link is redirected without firing event again', function (): void {
    ...
    $this->actingAs($user)->get($verificationUrl)
        ->assertRedirect(route('trading', absolute: false).'?verified=1');
